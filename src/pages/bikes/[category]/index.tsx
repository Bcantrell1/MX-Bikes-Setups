...
export default function BikesCategoryPage() {
    ...
    return (
        <div className="max-w-2xl mx-auto py-8 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
                ...
                {data?.map((bike) => (
                    <Link href={`/bikes/${category}/${bike.name}`} key={bike.id} className="w-full min-h-30  aspect-w-1 aspect-h-1 rounded-md overflow-hidden lg:h-30 aspect-none">
                            <div className="group relative cursor-pointer">
                                <div className='w-full min-h-30  aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-30 aspect-none'>
                                    <Image src={`/bikes/${bike.image}`} alt={bike.name || 'image'} className="w-full h-full object-center object-cover lg:w-full lg:h-full" width={400} height={275}/>
                                </div>
                            {bike.name}
                            </div>
                    </Link>
